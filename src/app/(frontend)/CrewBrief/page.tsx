...
function CrewBrief() {
  ...
  useEffect(() => {
    const fetchCrewBriefs = async () => {
      try {
        // Start with depth=0 to avoid database issues
        const response = await fetch("/api/crew-briefs?depth=0&limit=10");
        if (response.ok) {
          const data = await response.json();
          console.log("API Response:", data); // Debug log

          // Process briefs to get media URLs
          const briefsWithMedia = await Promise.all(
            (data.docs || []).map(async (brief: Record<string, unknown>) => {
              if (brief.banner && typeof brief.banner === "string") {
                brief.bannerUrl = await getMediaUrl(brief.banner);
              }
              return brief as unknown as CrewBrief;
            })
          );

          setCrewBriefs(briefsWithMedia);
        } else {
        ...
        {loading ? (
          ...
          <div className="flex flex-wrap justify-evenly gap-10">
            {crewBriefs.slice(0, 3).map((brief, index) => {
              console.log("Brief banner:", brief.banner); // Debug log
              console.log("Brief bannerUrl:", brief.bannerUrl); // Debug log

              // Use the processed bannerUrl or fallback
              const bannerUrl =
                brief.bannerUrl ||
                (typeof brief.banner === "object" ? brief.banner?.url : null) ||
                "/gulfstream.jpg";

              return (
                <Link key={brief.id || index} href={`/CrewBrief/${brief.id}`}>
                  <div
